...
WORKSPACE_LINKS: dict[str, str] = {
    ...
    NODE_IDENTITY: "/copilot?alert_id={alert_id}&q=screening",
...
def _priority_from_alert(alert: dict[str, Any], ml: dict[str, Any], hits: list[dict[str, Any]]) -> tuple[str, str]:
    if hits or ml.get("score", 0) >= 80 or alert.get("risk_level") == "high":
        return "critical", "fast-track"
    if ml.get("score", 0) >= 55 or alert.get("risk_level") == "medium":
        return "high", "elevated"
    if ml.get("score", 0) < 30:
        return "low", "routine"
...
def node_identity(state: InvestigationState, emit: _Emitter) -> None:
    alert = state.alert
    aid = alert.get("id", "")
    ...
    emit.emit(
        ...
        "completed",
        "OSINT_Search",
        (osint.get("reply") or "OSINT complete")[:280],
...
def node_investigator(state: InvestigationState, emit: _Emitter) -> None:
    alert = state.alert
    aid = alert.get("id", "")
    ...
    exposure = compute_exposure(aid) if aid else {
        "direct_exposure": 0.0,
    ...
    emit.emit(
        NODE_INVESTIGATOR,
        "completed",
        "KYT_Score + Travel Rule",
        f"{kyt['summary']} · Travel Rule {travel} · tags [{tags}]",
        40,
        skill_id="kyt-score",
    )
...
def node_arbiter(state: InvestigationState, emit: _Emitter) -> None:
    ...
    emit.emit(
        ...
        f" · {len(explainability)} explainability factor(s)",
        25,
...
def run_investigation(
    ...
    """Execute 4-node graph sequentially; returns triage-compatible result."""
    state = InvestigationState(alert=alert)
    emit = _Emitter(state, on_step, alert.get("id", ""))
    node_orchestrator(state, emit)
    node_identity(state, emit)
    node_investigator(state, emit)
